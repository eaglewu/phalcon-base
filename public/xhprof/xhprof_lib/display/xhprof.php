...
function print_flat_data($url_params, $title, $flat_data, $sort, $run1, $run2, $limit)
{
    ...
    print('<div class="table-box"><table border=1 cellpadding=2 cellspacing=1 width="90%" '
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2)
{
    ...
    if ($diff_mode) {
        ...
        echo "<div class='summary'> <ul>";

        foreach ($metrics as $metric) {
            echo '<li>';
            //echo "<span class='glyphicon glyphicon-asterisk'>".number_format($totals[$metric]).$possible_metrics[$metric][1]."</span>";
            echo "<span class='glyphicon glyphicon-asterisk'>".number_format($totals[$metric])."</span>";
            echo "<span class='glyphicon-class'>".str_replace("<br>", " ", stat_description($metric))."</span>";
        ...
        if ($display_calls) {
            echo "<li>";
            echo "<span class='glyphicon glyphicon-asterisk'>".number_format($totals['ct'])."</span>";
            echo "<span class='glyphicon-class'>Number of Function Calls</span>";
            echo "</li>";
        }
    ...
    if ($diff_mode) {
        ...
        if ($all) {
            ...
            $title = "Displaying top <span class='label label-primary'>$limit</span> functions: Sorted by <span class='label label-default'>$desc</span>";
        }
    }
                       ...
                       $symbol_info2 = NULL)
{
    ...
    if ($diff_mode) {
        ...
        print('<div class="table-box"><table border=1 cellpadding=2 cellspacing=1 width="30%" '
            . 'rules=rows bordercolor="#bdc7d8" align=center>' . "\n");
        print('<tr bgcolor="#bdc7d8" align=right>');
